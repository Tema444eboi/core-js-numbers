...
function roundToPowerOfTen(num, pow) {
  return pow === 0
    ? num
    : roundToPowerOfTen(Math.round(num / 10), pow - 1) * 10;
}
 ...
 *   10 => 55
 */
function getFibonacciNumber(index, a = 0, b = 1) {
  return index === 0 ? a : getFibonacciNumber(index - 1, b, a + b);
}
 ...
 *   1  => 1
 */
function getSumToN(n, res = 0) {
  return n === 0 ? res : getSumToN(n - 1, res + n);
}
 ...
 *   5   => 5  // 5
 */
function getSumOfDigits(num, res = 0) {
  return num === 0
    ? res
    : getSumOfDigits(Math.floor(num / 10), res + (num % 10));
}
 ...
 * 15 => 8
 */
function getCountOfOddNumbers(number, count = 0) {
